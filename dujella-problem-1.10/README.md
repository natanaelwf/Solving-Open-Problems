# Jordan Diophantine Tuples in $2 \times 2$ Integer Matrix Rings
...
is pairwise noncommuting and consists of involutions. Its pairwise Jordan products are all equal to $I_2$, while every scalar integer matrix is a square in $M_2(\mathbb{Z})$. Consequently, the same family is a Jordan $D(nI_2)$-family for every $n\in\mathbb{Z}$.

We also determine exactly when a scalar parameter admits a quadruple in the upper-triangular ring $\mathrm{UT}_2(\mathbb{Z})$: a Jordan $D(nI_2)$-quadruple exists if and only if

$$
n\not\equiv 2\pmod 4,
$$

equivalently if and only if $nI_2$ is a difference of two squares in $\mathrm{UT}_2(\mathbb{Z})$. Whenever one exists it can be chosen pairwise noncommuting. For every admissible $n$ except $n=1$ and $n=4$, the proof gives a countably infinite family. A separate construction covers those two parameters, and a final extension supplies infinite families whenever $N+a^2I_2$ is a square for some nonzero integer $a$, including noncentral parameters.

**Keywords.** Diophantine tuples; Jordan product; noncommutative rings; integer matrices; upper-triangular matrices; involutions.

## 1. Introduction

Let $R$ be a commutative ring and let $n\in R$. A classical $D(n)$-$r$-tuple is a set of distinct nonzero elements

$$
\{a_1,\ldots,a_r\}\subseteq R
$$

such that $a_i a_j+n$ is a square in $R$ whenever $i<j$; see [1] for background.

Problem 1.10 in Dujella's list asks for a noncommutative analogue and for nontrivial results concerning the existence of quadruples, naming the ring of $2\times2$ integer matrices as a test case [2]. The problem records the proposal of Dujella and Franušić to use the Jordan product

$$
A\circ B=\frac{AB+BA}{2}.
$$

Their motivating paper points more specifically toward upper-triangular integer matrices [3]. Whenever an integer $n$ is used below as a matrix parameter, it is identified with the central matrix $nI_2$. We distinguish throughout between the full ring $M_2(\mathbb{Z})$ and its upper-triangular subring $\mathrm{UT}_2(\mathbb{Z})$, because a square root in $M_2(\mathbb{Z})$ need not lie in $\mathrm{UT}_2(\mathbb{Z})$.
...
The first main result concerns the full matrix ring. For every $n\in\mathbb{Z}$, one countably infinite family supplies pairwise noncommuting Jordan $D(nI_2)$-tuples of every finite size, and every member is an involution in $\mathrm{GL}_2(\mathbb{Z})$. The proof rests on the identities
...
**Definition 2.1 (Integral Jordan $D(N)$-tuple).** A set of distinct nonzero elements

$$
\{A_1,\ldots,A_r\}\subseteq\mathcal{A}
$$

is a Jordan $D(N)$-$r$-tuple, or a $D^\circ(N)$-$r$-tuple, if for every $i<j$ there exists $R_{ij}\in\mathcal{A}$ such that
...
**Proposition 2.2 (Compatibility with the classical definition).** If $\mathcal{A}$ is commutative, then Definition 2.1 is exactly the classical $D(N)$ condition.

*Proof.* In a commutative ring, (1) becomes
...
**Remark 2.3 (Interpretation after adjoining $1/2$).** Equation (1) itself gives

$$
A_iA_j+A_jA_i=2\left(R_{ij}^2-N\right).
$$
...
is injective under the no-$2$-torsion hypothesis, and in this localization (1) becomes
...
Consequently, if $A,B,C$ are distinct and nonzero, then $\{A,B,C\}$ is a $D^\circ(N)$-triple.
...
**Lemma 3.1 (Scalar matrices are squares in $M_2(\mathbb{Z})$).** For every $q\in\mathbb{Z}$, define
...
3. For every $n\in\mathbb{Z}$, every finite subset of distinct members of $\mathcal{X}$ is a $D^\circ(nI_2)$-tuple in $M_2(\mathbb{Z})$.

Consequently, for every $n\in\mathbb{Z}$ and every $r\ge2$, there exists a pairwise noncommuting $D^\circ(nI_2)$-$r$-tuple consisting entirely of involutions in $\mathrm{GL}_2(\mathbb{Z})$.

*Proof.* For every $t\in\mathbb{Z}$,
...
so $X_t^{-1}=X_t$, proving the first assertion. For $s,t\in\mathbb{Z}$,
...
Here $E_{ij}$ denotes the standard $2\times2$ matrix unit. If $s\ne t$, equation (4) is nonzero, proving the second assertion.

Fix $n\in\mathbb{Z}$. For distinct $s,t$, equation (3) gives $X_s\circ X_t=I_2$. By Lemma 3.1,

$$
X_s\circ X_t+nI_2=(n+1)I_2=Q_{n+1}^2.
$$

Thus every pair satisfies Definition 2.1, with the same root $Q_{n+1}$. This proves the third assertion and the final conclusion.
...
**Corollary 3.4 (An explicit quadruple for every integer parameter).** For every $n\in\mathbb{Z}$, the matrices
...
form a pairwise noncommuting $D^\circ(nI_2)$-quadruple in $M_2(\mathbb{Z})$. For each pair, one may take
...
**Corollary 3.5 (No finite upper bound).** For every $n\in\mathbb{Z}$, the cardinalities of pairwise noncommuting $D^\circ(nI_2)$-tuples in $M_2(\mathbb{Z})$ are unbounded, even when every element is required to be an involution in $\mathrm{GL}_2(\mathbb{Z})$.
...
Although every $X_t$ from Theorem 3.2 belongs to $\mathrm{UT}_2(\mathbb{Z})$, the roots $Q_{n+1}$ used there generally do not. The diagonal entries provide an immediate bridge back to ordinary integer square conditions.

**Lemma 4.1 (Diagonal projection).** Let $\{A_1,\ldots,A_r\}$ be a $D^\circ(nI_2)$-tuple in $\mathrm{UT}_2(\mathbb{Z})$, and write
...
**Proposition 4.2 (A congruence obstruction).** If $n\equiv2\pmod4$, then every $D^\circ(nI_2)$-tuple in $\mathrm{UT}_2(\mathbb{Z})$ has at most three elements. In particular, no Jordan $D(nI_2)$-quadruple exists in $\mathrm{UT}_2(\mathbb{Z})$.

*Proof.* There is nothing to prove when $r\le3$, so suppose $r\ge4$. Use the integers $a_i$ from Lemma 4.1. For each $i<j$, the integer $a_i a_j+n$ is a square, hence is congruent to $0$ or $1$ modulo $4$.
...
**Proposition 4.4 (A noncommuting lift of classical tuples).** Let $\{c_1,\ldots,c_r\}\subset\mathbb{Z}$ be a classical $D(n)$-$r$-tuple and define
...
Then $\{A_1,\ldots,A_r\}$ is a pairwise noncommuting $D^\circ(nI_2)$-$r$-tuple in $\mathrm{UT}_2(\mathbb{Z})$.

*Proof.* The matrices are distinct and nonzero because the $c_i$ are distinct and nonzero. For $i\ne j$, direct multiplication gives
...
**Theorem 4.5 (Scalar-parameter classification in $\mathrm{UT}_2(\mathbb{Z})$).** For $n\in\mathbb{Z}$, the following are equivalent.

1. A Jordan $D(nI_2)$-quadruple exists in $\mathrm{UT}_2(\mathbb{Z})$.
2. $n\not\equiv2\pmod4$.
3. The scalar matrix $nI_2$ is a difference of two squares in $\mathrm{UT}_2(\mathbb{Z})$.

Whenever these conditions hold, the quadruple can be chosen pairwise noncommuting. Moreover, if $n\not\equiv2\pmod4$ and $n\notin\{1,4\}$, then $\mathrm{UT}_2(\mathbb{Z})$ contains a countably infinite pairwise noncommuting $D^\circ(nI_2)$-family.

*Proof.* The implication (1) $\Rightarrow$ (2) follows from Proposition 4.2. For (2) $\Rightarrow$ (1), first suppose that $n\not\equiv2\pmod4$ and $n\notin\{1,4\}$. If $n$ is odd, take
...
It remains to treat $n=1$ and $n=4$. Write

$$
F=\{c_1,c_2,c_3,c_4\}=\{1,3,8,120\}
$$

for Fermat's classical $D(1)$-quadruple. Directly,

$$
\{c_i c_j+1:i<j\}
=\{2^2,3^2,5^2,11^2,19^2,31^2\}.
$$

For $m\in\{1,2\}$, the scaled set $mF$ is a $D(m^2)$-quadruple, since
...
It remains to verify the equivalence of (2) and (3). If $n\not\equiv2\pmod4$, the preceding formulas represent $n=b^2-a^2$ for some $a,b\in\mathbb{Z}$; for $n=1$ or $n=4$, one may allow $a=0$. Hence

$$
nI_2=(bI_2)^2-(aI_2)^2
$$

in $\mathrm{UT}_2(\mathbb{Z})$. Conversely, if

$$
nI_2=U^2-V^2
$$

with $U,V\in\mathrm{UT}_2(\mathbb{Z})$, comparison of either diagonal entry expresses $n$ as a difference of two integer squares. Such a difference is never congruent to $2$ modulo $4$. Thus (2) and (3) are equivalent.

**Remark 4.6 (The distinction between the two ambient rings).** For $n\equiv2\pmod4$, Theorem 3.2 gives a countably infinite pairwise noncommuting $D^\circ(nI_2)$-family in the full ring $M_2(\mathbb{Z})$, whereas Proposition 4.2 rules out even a quadruple in $\mathrm{UT}_2(\mathbb{Z})$. Thus the non-upper-triangular roots in the full-ring construction are essential in this congruence class.
...
**Proposition 5.1.** Let $a\in\mathbb{Z}\setminus\{0\}$ and $N\in M_2(\mathbb{Z})$. If
...
The doubled identity in Definition 2.1 is an integral form of the Jordan-square condition. It agrees with the classical $D(N)$ definition in commutative rings without additive $2$-torsion, and the regular-extension calculation requires no commutativity.

For the full ring $M_2(\mathbb{Z})$, Theorem 3.2 gives substantially more than one quadruple: for every $n\in\mathbb{Z}$ and every $r\ge2$, there is a pairwise noncommuting $D^\circ(nI_2)$-$r$-tuple of integral involutions, always selected from the same countably infinite family.

For the upper-triangular ring, Theorem 4.5 gives an exact answer for scalar parameters: a Jordan $D(nI_2)$-quadruple exists precisely when $n\not\equiv2\pmod4$, equivalently precisely when $nI_2$ is a difference of two squares in $\mathrm{UT}_2(\mathbb{Z})$, and it may always be chosen pairwise noncommuting. The proof supplies infinite families for every admissible $n$ except $1$ and $4$, for which explicit pairwise noncommuting quadruples are obtained by lifting Fermat's classical example.
...
3. A. Dujella and Z. Franušić. “Differences of Two Squares of Upper-Triangular $2\times2$ Integer Matrices.” arXiv:2604.23404v2 [math.NT], 8 May 2026.
